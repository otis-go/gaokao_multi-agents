...
3. run.py does NOT import DataLoader for pre-validation (only for unit_id list)
...
def check_cli_independence(result: IndependenceCheckResult, verbose: bool = False):
    """
    Check that run.py does not import implementation details for pre-validation.
    ...
    """
    cli_path = PROJECT_ROOT / "run.py"

    if not cli_path.exists():
        result.add_warning("run.py not found")
