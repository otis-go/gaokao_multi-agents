...
class BaselineQuestion:
    ...
    material_type: str  # Chinese labels for expository or argumentative text.
    stem: str
    material_text: str
...
def _parse_essay_answer(answer_text: str) -> Tuple[Optional[List[Dict[str, Any]]], Optional[int]]:
    ...
    # Try to parse "Example 1/2" format
        # Pattern: numbered examples followed by question and answer.
...
class BaselineEvaluator:
    ...
    def _match_dimension_ids(
        ...
        # Real exam data may store only the short label; match it to the full dimension name.
        def match_label_to_dim_name(dim_id: str, label: str) -> Optional[str]:
            ...
            for dim_def in self._abc_prompts:
                if dim_def.get("id") == dim_id:
                    ...
                    # Example: a short label can match the suffix of a full dimension name.
        ...
        # level1 is category info and should not be used for fine-grained dimension matching.
        # level2 contains specific dimension labels and should match dimension_name precisely.
    ...
    def build_stage2_record(
        ...
        Args:
            ...
            dim_mode: Dimension mode - "gk_only" / "cs_only" / "gk" / "cs" / "gk+cs"
        ...
        elif dim_mode in ("gk+cs", "both", "all"):
            # Keep both GK and CS dimensions for dual pedagogical evaluation.
            pass
        else:
            print(f"[BaselineEvaluator] [WARN] Unknown dim_mode={dim_mode}, fallback to keep all GK/CS dimensions")
    ...
    def get_recent_years_unit_ids(self, start_year: int = 2020, end_year: int = 2025) -> List[str]:
        ...
        for unit_id, raw_data in self._raw_materials.items():
            ...
            # Extract year from source; the year is expected at the beginning.
    ...
    def get_gold_dimensions(self, unit_id: str, dim_mode: str = "gk") -> Tuple[List[str], Dict[str, List[str]], Dict[str, List[str]]]:
        ...
        Args:
            unit_id: Question ID
            dim_mode: Dimension mode - "gk" / "cs" / "gk+cs"
